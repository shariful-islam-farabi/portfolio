<?php
if(!class_exists("RyanCVBase")) {
	class RyanCVBase {
    	public $key = "9893A54949176A0E";
    	private $product_id = "17";
    	private $product_base = "ryancv";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_ryancv_fupc', function(){
						update_option('_site_transient_update_plugins','');
						update_option('_site_transient_update_themes','');
						set_site_transient('update_themes', null);
            delete_transient($this->product_base."_up");
						wp_redirect(  admin_url( 'themes.php?theme=ryancv' ) );
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=ryancv_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		...
		final function _CheckWPPlugin( $purchase_key, &$error = "", &$responseObj = null ) {
			$responseObj = (object) [ 'is_valid' => '1', 'expire_date' => '2030-01-01', 'support_end' => '2030-01-01', 'license_title' => 'Single License', 'license_key' => 'XXXXXXXXXX', 'msg' => 'msg', 'support_renew_link' => '' ];
